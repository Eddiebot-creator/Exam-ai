...
class _CalmStudentAppState extends State<CalmStudentApp> {
  ...
  final mainTabs = const [
    ...
    AppTab('Tools', Icons.apps_rounded),
  ...
  Future<void> refresh() async {
    ...
    try {
      final results = await Future.wait([widget.api.notes(userId), widget.api.history(userId), widget.api.progress(userId)]);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: desktop ? null : NavigationBar(
        ...
        onDestinationSelected: (value) => setState(() => index = [0,1,2,3,7][value]),
        destinations: const [
