...
def chat(p: dict, db: Session = Depends(get_db)):
    ...
    weak = mem.weak_topics if mem and mem.weak_topics else ["your current topic"]
    context = note.extracted_text[:350] if note else "No uploaded note context yet."

    answer = (
        f"{coach_message(weak)}\n\n"
        f"Your question: {msg}\n\n"
        f"Step-by-step answer:\n"
        f"1. Identify the key idea.\n"
        f"2. Connect it to your notes.\n"
        f"3. Work through one example.\n"
        f"4. Test yourself with a short quiz.\n\n"
        f"Note context: {context}"
    )
    ...
    return {"answer": answer, "context_note_id": note.id if note else None}
