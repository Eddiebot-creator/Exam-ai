...
from services.autonomous_orchestrator import process_learning_event, adaptive_home, tutor_context, seed_onboarding_context
...
def onboarding_context(payload: dict, db: Session = Depends(get_db)):
    return seed_onboarding_context(
        db,
        int(payload.get("user_id", 1)),
        payload.get("struggle", ""),
        payload.get("course", ""),
    )
